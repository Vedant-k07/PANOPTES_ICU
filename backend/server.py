...
from services.clinical_decision_service import ClinicalDecisionService
...
async def calculate_ranson_score(input_data: Ranson_Input):
    ...
    try:
        ...
        logger.error(f"Ranson calculation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

@api_router.post("/scoring/saps")
async def calculate_saps_score(input_data: SAPS_Input):
    """Calculate SAPS II Score
    
    Simplified Acute Physiology Score for ICU mortality prediction
    """
    try:
        ...
        logger.error(f"SAPS calculation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

@api_router.post("/scoring/mods")
async def calculate_mods_score(input_data: MODS_Input):
    ...
    try:
        ...
        logger.error(f"MODS calculation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

@api_router.post("/scoring/murray")
async def calculate_murray_score(input_data: Murray_Input):
    """Calculate Murray Score
    
    Lung Injury Score for ARDS assessment
    """
    try:
        ...
        logger.error(f"Murray calculation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))

@api_router.post("/scoring/alvarado")
async def calculate_alvarado_score(input_data: Alvarado_Input):
    """Calculate Alvarado Score
    
    Acute appendicitis probability assessment
    """
    try:
        ...
        logger.error(f"Alvarado calculation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
...
@api_router.post("/clinical/treatment-protocol")
async def get_treatment_protocol(
    patient_id: str,
    condition: str,
    healthcare_associated: bool = False,
    immunocompromised: bool = False
):
    """Get evidence-based treatment protocol recommendations
    
    Supports: SEPSIS, ARDS, AKI, SHOCK
    """
    try:
        clinical_data = {
        ...
        result = clinical_decision_service.get_treatment_protocol(
            patient_id, condition, clinical_data
        )
        return result
    except Exception as e:
        logger.error(f"Treatment protocol error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/clinical/fluid-responsiveness")
async def predict_fluid_responsiveness(
    patient_id: str,
    stroke_volume_variation: Optional[float] = None,
    pulse_pressure_variation: Optional[float] = None,
    passive_leg_raise_response: Optional[float] = None,
    ivc_collapsibility: Optional[float] = None,
    cvp: Optional[float] = None,
    mechanical_ventilation: bool = False,
    tidal_volume_ml_kg: float = 6
):
    """Predict fluid responsiveness using dynamic hemodynamic parameters"""
    try:
        hemodynamic_data = {
            "stroke_volume_variation": stroke_volume_variation,
            "pulse_pressure_variation": pulse_pressure_variation,
            "passive_leg_raise_response": passive_leg_raise_response,
            "ivc_collapsibility": ivc_collapsibility,
            "cvp": cvp,
            "mechanical_ventilation": mechanical_ventilation,
            "tidal_volume_ml_kg": tidal_volume_ml_kg
        }
        result = clinical_decision_service.predict_fluid_responsiveness(
            patient_id, hemodynamic_data
        )
        return result
    except Exception as e:
        logger.error(f"Fluid responsiveness error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/clinical/antimicrobial")
async def get_antimicrobial_recommendation(
    patient_id: str,
    infection_site: str,
    suspected_organisms: List[str] = [],
    allergies: List[str] = [],
    creatinine_clearance: Optional[float] = None,
    days_on_antibiotics: int = 0
):
    """Get antimicrobial stewardship recommendations"""
    try:
        infection_data = {
            "infection_site": infection_site,
            "suspected_organisms": suspected_organisms,
            "allergies": allergies,
            "creatinine_clearance": creatinine_clearance,
            "days_on_antibiotics": days_on_antibiotics
        }
        result = clinical_decision_service.get_antimicrobial_recommendation(
            patient_id, infection_data
        )
        return result
    except Exception as e:
        logger.error(f"Antimicrobial recommendation error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/clinical/weaning-assessment")
async def assess_weaning_readiness(
    patient_id: str,
    weaning_type: str,
    ...
    """Assess ventilator or vasopressor weaning readiness"""
    try:
        clinical_data = {
            "pf_ratio": pf_ratio or 200,
            "peep": peep or 8,
            "map": map_val or 70,
            "gcs": gcs or 14,
            "norepinephrine_dose": norepinephrine_dose or 0,
            "map_stable_4h": True,
            "acute_phase_resolved": True
        }
        result = clinical_decision_service.assess_weaning_readiness(
            patient_id, weaning_type, clinical_data
        )
        return result
    except Exception as e:
        logger.error(f"Weaning assessment error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/clinical/nutrition")
async def get_nutrition_recommendation(
    ...
    try:
        ...
        result = clinical_decision_service.get_nutrition_recommendation(
            patient_id, nutrition_data
        )
        return result
    except Exception as e:
        logger.error(f"Nutrition recommendation error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
@api_router.post("/monitoring/hemodynamic-prediction")
async def predict_hemodynamic_instability(
    patient_id: str,
    ...
    vital_signs should be a list of dicts with: hr, sbp, dbp, map (last 6+ hours)
    """
    try:
        lab_values = {}
        if lactate:
            lab_values["lactate"] = lactate
        if hemoglobin:
            lab_values["hemoglobin"] = hemoglobin
        
        result = advanced_monitoring_service.predict_hemodynamic_instability(
            patient_id, vital_signs, lab_values if lab_values else None
        )
        return result
    except Exception as e:
        logger.error(f"Hemodynamic prediction error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/monitoring/vital-patterns")
async def analyze_vital_patterns(
    patient_id: str,
    hr: int,
    sbp: int,
    dbp: int,
    ...
    try:
        ...
        result = advanced_monitoring_service.analyze_vital_patterns(
            patient_id, vital_signs
        )
        return result
    except Exception as e:
        logger.error(f"Vital pattern analysis error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/monitoring/delirium-risk")
async def assess_delirium_risk(
    patient_id: str,
    age: int,
    gcs: int = 15,
    dementia: bool = False,
    ...
    """Assess delirium risk and get prevention strategies"""
    try:
        assessment_data = {
            "age": age,
            "gcs": gcs,
        ...
        result = advanced_monitoring_service.assess_delirium_risk(
            patient_id, assessment_data
        )
        return result
    except Exception as e:
        logger.error(f"Delirium risk assessment error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/monitoring/multi-organ-trajectory")
async def monitor_organ_trajectory(
    patient_id: str,
    pf_ratio: float = 300,
    map_val: float = 75,
    vasopressor_dose: float = 0,
    creatinine: float = 1.0,
    urine_output: float = 0.5,
    bilirubin: float = 1.0,
    gcs: int = 15,
    platelets: float = 200
):
    ...
    try:
        organ_data = {
            ...
            "urine_output_ml_kg_h": urine_output,
            "bilirubin": bilirubin,
            "gcs": gcs,
            "platelets": platelets
        }
        result = advanced_monitoring_service.monitor_multi_organ_trajectory(
            patient_id, organ_data
        )
        return result
    except Exception as e:
        logger.error(f"Organ trajectory error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@api_router.post("/monitoring/sleep-quality")
async def assess_sleep_quality(
    patient_id: str,
    avg_noise_db: float = 45,
    night_light_lux: float = 50,
    care_interruptions_per_night: int = 8,
    pain_controlled: bool = True,
    anxiety_level: str = "low"
):
    """Assess ICU patient sleep quality and get interventions"""
    try:
        sleep_data = {
            "avg_noise_db": avg_noise_db,
            "night_light_lux": night_light_lux,
            "care_interruptions_per_night": care_interruptions_per_night,
            "pain_controlled": pain_controlled,
            "anxiety_level": anxiety_level
        }
        result = advanced_monitoring_service.assess_sleep_quality(
            patient_id, sleep_data
        )
        return result
    except Exception as e:
        logger.error(f"Sleep quality assessment error: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
